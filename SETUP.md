# 🚀 Guía de Setup - Academe

Esta guía te ayudará a configurar el proyecto Academe desde cero.
...
```bash
git clone https://github.com/Andreu-Lechuga/Academe.git
cd Academe
```
...
Deberías ver:
- `academe_postgres` en puerto 5432
- `academe_redis` en puerto 6379
...
docker exec -it academe_postgres psql -U academe -d academe
...
```bash
docker exec -i academe_postgres psql -U academe -d academe < backend/migrations/001_init_schemas.sql
docker exec -i academe_postgres psql -U academe -d academe < backend/migrations/002_auth.sql
docker exec -i academe_postgres psql -U academe -d academe < backend/migrations/003_calendar.sql
```
...
Deberías ver:
```
INFO: 🚀 Iniciando Academe API Server...
...
```bash
# Verificar conexión
docker exec -it academe_postgres psql -U academe -d academe -c "\dn"
```
